...
def clean(x):
    # replace evrything thats not an alphabet with a space
    x = re.sub(r'[^a-zA-Z ]', ' ', x)
...
def extract_all_reviews(url, clean_reviews, org_reviews, customernames, commentheads, ratings):
    ...
    ratings_ = page_html.find_all('div', {'class': [
                                  '_3LWZlK _1BLPMq', '_3LWZlK _32lA32 _1BLPMq', '_3LWZlK _1rdVr6 _1BLPMq']})
...
def result():
    ...
    price = page_html.find_all('div', {'class': '_30jeq3 _16Jk6d'})[
        0].get_text()
    ...
    while True:
        ...
        extract_all_reviews(url2, clean_reviews, org_reviews,
    ...
    wc = WordCloud(width=1400, height=800, stopwords=wcstops,
                   background_color='white').generate(for_wc)
